...
if (!(verts == Number(res.headers.get("n-vertex"))) || !(CONFIG.RAW_FLOAT_PER_SPLAT == Number(res.headers.get("n-channels")))) {
    console.log("Vertex:", verts, Number(res.headers.get("n-vertex")))
    console.log("Channels:", CONFIG.RAW_FLOAT_PER_SPLAT, Number(res.headers.get("n-channels")))
